...
import {
  User, ShieldCheck, Clock, AlertCircle, FileText,
  MapPin, Key, Edit, LogOut, CheckCircle, Eye, EyeOff, Loader2, X, Download, Trash2, Upload, Briefcase
} from "lucide-react";
...
export default function UserDashboard() {
  ...
  // Tabs: "overview", "settings", "coordinator", "payment_details"
  const [activeTab, setActiveTab] = useState<"overview" | "settings" | "coordinator" | "payment_details">("overview");
  ...
  const fetchProfileData = async () => {
    ...
    try {
      ...
      setError((err as Error).message || "Session expired or failed to load profile.");
      if ((err as Error).message?.toLowerCase().includes("denied") || (err as Error).message?.toLowerCase().includes("token")) {
        handleLogout();
      }
    } finally {
      setLoading(false);
  ...
  const handleUserPaymentUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Upload the image
      const presigned = await getPresignedUploadUrl(paymentReceiptFile.name, paymentReceiptFile.type);
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-6 print:hidden">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold border rounded-xl hover:bg-green-50 bg-white transition-all shadow-sm text-green-700 border-green-200"
                >
